# 4Gamers Route Index

## Namespace
- Namespace: `4gamers`
- Display Name: `4Gamers`
- URL: `www.4gamers.com.tw`
- Language: `_None_`
- Aliases: `4gamers, 4gamers.com.tw, www, www.4gamers.com.tw`
- Route Count: `4`

## Routes

### 最新消息
- Route ID: `4gamers:/4gamers/`
- Route Path: `/4gamers/`
- File: `docs/routes/4gamers/4gamers.md`
- File Name: `4gamers.md`
- Categories: `game`
- Maintainers: `TonyRL`

### 分类
- Route ID: `4gamers:/4gamers/category/:category`
- Route Path: `/4gamers/category/:category`
- File: `docs/routes/4gamers/4gamers-category-category.md`
- File Name: `4gamers-category-category.md`
- Categories: `game`
- Maintainers: `TonyRL`

### 标签
- Route ID: `4gamers:/4gamers/tag/:tag`
- Route Path: `/4gamers/tag/:tag`
- File: `docs/routes/4gamers/4gamers-tag-tag.md`
- File Name: `4gamers-tag-tag.md`
- Categories: `game`
- Maintainers: `hoilc`

### 主題
- Route ID: `4gamers:/4gamers/topic/:topic`
- Route Path: `/4gamers/topic/:topic`
- File: `docs/routes/4gamers/4gamers-topic-topic.md`
- File Name: `4gamers-topic-topic.md`
- Categories: `game`
- Maintainers: `bestpika`
