# Japan Local Government Route Index

## Namespace
- Namespace: `lg`
- Display Name: `Japan Local Government`
- URL: `lg.jp`
- Language: `_None_`
- Aliases: `japan local government, lg, lg.jp`
- Route Count: `3`

## Routes

### 大阪市 食品営業許可 新規
- Route ID: `lg:/lg/osaka/food-permit`
- Route Path: `/lg/osaka/food-permit`
- File: `docs/routes/lg/lg-osaka-food-permit.md`
- File Name: `lg-osaka-food-permit.md`
- Categories: `government`
- Maintainers: `pseudoyu`

### 東京都 飲食店営業許可 新規
- Route ID: `lg:/lg/tokyo/food-permit/:ward?`
- Route Path: `/lg/tokyo/food-permit/:ward?`
- File: `docs/routes/lg/lg-tokyo-food-permit-ward.md`
- File Name: `lg-tokyo-food-permit-ward.md`
- Categories: `government`
- Maintainers: `pseudoyu`

### 東京都統計年鑑 駅別乗降車人員
- Route ID: `lg:/lg/tokyo/rail-ridership/:table?`
- Route Path: `/lg/tokyo/rail-ridership/:table?`
- File: `docs/routes/lg/lg-tokyo-rail-ridership-table.md`
- File Name: `lg-tokyo-rail-ridership-table.md`
- Categories: `other`
- Maintainers: `pseudoyu`
