# AcFun Route Index

## Namespace
- Namespace: `acfun`
- Display Name: `AcFun`
- URL: `www.acfun.cn`
- Language: `_None_`
- Aliases: `acfun, acfun.cn, www, www.acfun.cn`
- Route Count: `4`

## Routes

### 文章
- Route ID: `acfun:/acfun/article/:categoryId/:sortType?/:timeRange?`
- Route Path: `/acfun/article/:categoryId/:sortType?/:timeRange?`
- File: `docs/routes/acfun/acfun-article-categoryid-sorttype-timerange.md`
- File Name: `acfun-article-categoryid-sorttype-timerange.md`
- Categories: `anime`
- Maintainers: `TonyRL`

### 番剧
- Route ID: `acfun:/acfun/bangumi/:id/:embed?`
- Route Path: `/acfun/bangumi/:id/:embed?`
- File: `docs/routes/acfun/acfun-bangumi-id-embed.md`
- File Name: `acfun-bangumi-id-embed.md`
- Categories: `anime`
- Maintainers: `xyqfer`

### 用户文章
- Route ID: `acfun:/acfun/user/article/:uid`
- Route Path: `/acfun/user/article/:uid`
- File: `docs/routes/acfun/acfun-user-article-uid.md`
- File Name: `acfun-user-article-uid.md`
- Categories: `anime`
- Maintainers: `tiaod`

### 用户投稿
- Route ID: `acfun:/acfun/user/video/:uid/:embed?`
- Route Path: `/acfun/user/video/:uid/:embed?`
- File: `docs/routes/acfun/acfun-user-video-uid-embed.md`
- File Name: `acfun-user-video-uid-embed.md`
- Categories: `anime`
- Maintainers: `wdssmq`
